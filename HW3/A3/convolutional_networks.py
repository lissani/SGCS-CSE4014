...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        x_padded = torch.nn.functional.pad(x, (pad_size, pad_size, pad_size,pad_size))
            ...
            for c in range(C):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        # 최대값 선택
                        out[n, c, i, j] = x[n, c, h_start:h_end, w_start:w_end].max()
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W1'] = torch.randn(
            num_filters, C, filter_size, filter_size, dtype=dtype, device=device) * weight_scale
        self.params['b1'] = torch.zeros(num_filters, dtype=dtype, device=device)

        conv_output_dim = (H // 2) * (W // 2) * num_filters
        self.params['W2'] = torch.randn(
            conv_output_dim, hidden_dim, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        dout, grads['W3'], grads['b3'] = Linear.backward(dscores, cache_fc2)
        dout, grads['W2'], grads['b2'] = Linear_ReLU.backward(dout, cache_fc1)
        _, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dout, cache_conv)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i, F in enumerate(num_filters):
            ...
            if weight_scale == 'kaiming':
                self.params[f'W{i+1}'] = kaiming_initializer(
                    prev_dim, F, K=3, relu=True, dtype=dtype, device=device
                )
            else:
                self.params[f'W{i+1}'] = weight_scale * torch.randn(
                    F, prev_dim, 3, 3, dtype=dtype, device=device
                )
        ...
        pool_count = len(max_pools)
        H_final = H // (2 ** pool_count)
        W_final = W // (2 ** pool_count)
        fc_input_dim = prev_dim * H_final * W_final

        self.params[f'W{self.num_layers}'] = kaiming_initializer(
            fc_input_dim, num_classes, relu=False, dtype=dtype, device=device
        ) if weight_scale == 'kaiming' else weight_scale * torch.randn(
            fc_input_dim, num_classes, dtype=dtype, device=device
        )
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if i - 1 in self.max_pools:
                current_pool_param = pool_param
            else:
                current_pool_param = {'pool_height': 1, 'pool_width': 1, 'stride': 1}

            if self.batchnorm:
                gamma, beta = self.params[f'gamma{i}'], self.params[f'beta{i}']
                scores, cache = Conv_BatchNorm_ReLU_Pool.forward(
                    scores, W, b, gamma, beta, conv_param, self.bn_params[i - 1],
                    current_pool_param
                )
            else:
                scores, cache = Conv_ReLU_Pool.forward(
                    scores, W, b, conv_param, current_pool_param
                )
        ...
        # Backward pass
        grads = {}
        dscores, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, caches.pop())
        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']

        for i in range(self.num_layers - 1, 0, -1):
            cache = caches.pop()
            if self.batchnorm:
                dscores, dW, db, dgamma, dbeta = Conv_BatchNorm_ReLU_Pool.backward(dscores, cache)
                grads[f'gamma{i}'], grads[f'beta{i}'] = dgamma, dbeta
            else:
                dscores, dW, db = Conv_ReLU_Pool.backward(dscores, cache)
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    weight_scale = 'kaiming'  # Use Kaiming initialization for weights
    batch_size = 128  # Batch size for training
    num_epochs = 10  # Number of epochs (adjust for 60 seconds limit)

    # Instantiate the model
    model = DeepConvNet(
        input_dims=(3, 32, 32),  # CIFAR-10 input dimensions
        num_classes=10,  # Number of classes in CIFAR-10
        num_filters=[16, 32, 64],  # Convolutional filters for each layer
        max_pools=[0, 1, 2],  # Pooling after specific layers
        batchnorm=True,  # Use batch normalization
        weight_scale=weight_scale,  # Kaiming initialization
    ...
    solver = Solver(
        ...
        update_rule=adam,  # Use Adam optimizer
        optim_config={
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        # Replace "pass" statement with your code
        fan_in = Din  # 입력 차원
        weight_scale = (gain / fan_in) ** 0.5  # Kaiming 초기화의 스케일
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        # Gradient for normalized data
        dx_hat = dout * gamma

        # Gradients for mean and variance
        dvar = torch.sum(dx_hat * (x - mean) * -0.5 * torch.pow(var + eps, -1.5), dim=0)
    ...
    def backward_alt(dout, cache):
        ...
        # Simplified gradient for input
        dx_hat = dout * gamma
        dx = (1. / N) * (1. / torch.sqrt(var + eps)) * (
