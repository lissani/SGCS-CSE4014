...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dx_flat = torch.matmul(dout, w.T)
        dx = dx_flat.reshape(x.shape)

        dw = torch.matmul(x_flat.T, dout)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        h_relu, cache_fc_relu = Linear_ReLU.forward(X, W1, b1)
        scores, cache_fc2 = Linear.forward(h_relu, W2, b2)
        ...
        dh_relu, grads['W2'], grads['b2'] = Linear.backward(dscores, cache_fc2)
        grads['W2'] += 2*reg*W2

        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dh_relu, cache_fc_relu)
        grads['W1'] += 2*reg*W1
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            self.params[f'W{i+1}']=torch.randn(layer_dims[i], layer_dims[i+1], dtype=dtype, device=device)*weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.use_dropout:
                scores, do_cache = Dropout.forward(scores, self.dropout_param)
                dropout_caches.append(do_cache)
        
        W_final, b_final = self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}']
        scores, final_cache = Linear.forward(scores, W_final, b_final)
        ...
        dhidden, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}']=Linear.backward(dscores, caches.pop())
        grads[f'W{self.num_layers}']+=2*self.reg*self.params[f'W{self.num_layers}']

        for i in reversed(range(1,self.num_layers)):
            if self.use_dropout:
                dhidden = Dropout.backward(dhidden, dropout_caches.pop())
            dhidden, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dhidden, caches.pop())
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand(x.shape, device=x.device)>p).float()
            out = x*mask / (1-p)
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
