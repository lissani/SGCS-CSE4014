...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x[:2, :2] = 0  # 첫 두 행, 첫 두 열에 0 할당
    x[:2, 1:2] = 1  # 첫 두 행, 두 번째 열에 1 할당
    x[:2, 2:6] = 2  # 첫 두 행, 세 번째부터 여섯 번째 열에 2 할당
    
    # Next two rows
    x[2:4, :2] = 3  # 세 번째와 네 번째 행, 첫 두 열에 3 할당
    x[2:4, 1:2] = 4  # 세 번째와 네 번째 행, 두 번째 열에 4 할당
    x[2:4, 2:4] = 3  # 세 번째와 네 번째 행, 세 번째 열에 3 할당
    x[2:4, 3:4] = 4  # 세 번째와 네 번째 행, 네 번째 열에 4 할당
    x[2:4, 4:6] = 5
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for i in range(B):
        result.append(torch.matmul(x[i], y[i]))

    z = torch.stack(result)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    column_diff = x - column_mean 
    column_variance = torch.sum(column_diff ** 2, dim=0) / M
    column_std = torch.sqrt(column_variance)  
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    cumulative_lengths = torch.cumsum(ls, dim=0)

    split_tensors = torch.split(concatenated, ls.tolist())

    y = torch.tensor([t.mean() for t in split_tensors])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    for i, unique_value in enumerate(uniques):
        indices[i] = torch.nonzero(x == unique_value, as_tuple=False).min()
