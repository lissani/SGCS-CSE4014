...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
        train_sample_flat = x_train[i].flatten()

        diff = train_sample_flat - x_test.flatten(start_dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_flat = x_train.view(num_train, -1)  # Shape: (num_train, D)
    x_test_flat = x_test.view(num_test, -1)     # Shape: (num_test, D)
    
    train_sq_norms = torch.sum(x_train_flat ** 2, dim=1, keepdim=True)  # Shape: (num_train, 1)
    test_sq_norms = torch.sum(x_test_flat ** 2, dim=1, keepdim=True).t()  # Shape: (1, num_test)
    
    inner_product = torch.matmul(x_train_flat, x_test_flat.t())  # Shape: (num_train, num_test)
    
    dists = train_sq_norms + test_sq_norms - 2 * inner_product
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        _, topk_indices = torch.topk(dists[:, j], k=k, largest=False)

        nearest_labels = y_train[topk_indices]
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            accuracy = torch.mean((y_val_pred == y_val_fold).float()).item()
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracies in k_to_accuracies.items():
        ...
        if mean_accuracy > max_mean_accuracy or (mean_accuracy == max_mean_accuracy and k < best_k):
            best_k = k  
