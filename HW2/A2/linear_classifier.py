...
def svm_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    binary = (margins > 0).float()  # (N, C)
    
    row_sum = binary.sum(dim=1) 
    binary[torch.arange(num_train), y] -= row_sum  
    
    binary = binary.to(X.dtype)
    dW = X.t().mm(binary) / num_train  # (D, N) @ (N, C) -> (D, C)
    dW += 2 * reg * W  
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 1e-2, 1e-1]  # 0.001, 0.01, 0.1
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = X[i].view(1, -1).mm(W)
        scores = scores.squeeze()  # (C,)

        scores -= scores.max()  # 가장 큰 값 빼기

        exp_scores = torch.exp(scores)  # e^f_j
        probs = exp_scores / exp_scores.sum()  # Softmax 확률
...
def softmax_loss_vectorized(
    ...
    loss = -torch.log(correct_class_probs).sum() / num_train
    loss += reg * torch.sum(W * W)

    dprobs = probs.clone()
    dprobs[torch.arange(num_train), y] -= 1 
    dW = X.t().mm(dprobs) / num_train  
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-3, 5e-3, 1e-2, 5e-2]  # 0.001, 0.005, 0.01, 0.05
