...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_raw = X.mm(W1) + b1  # (N, H)

    hidden = hidden_raw.clone()
    hidden[hidden < 0] = 0  # ReLU: max(0, x)

    scores = hidden.mm(W2) + b2  # (N, C)   
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    grads["W2"] = h1.t().mm(dprobs) + 2 * reg * W2
    grads["b2"] = dprobs.sum(dim=0)

    dhidden = dprobs.mm(W2.t())
    dhidden[h1 <= 0] = 0  # ReLU backprop
